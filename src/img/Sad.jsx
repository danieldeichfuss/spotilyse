import React from "react";

export default function Sad() {
  return (
    <svg
      className="sad"
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 47.5 47.5"
      style={{ enableBackground: "new 0 0 47.5 47.5" }}
      version="1.1"
      id="svg2"
    >
      <defs id="defs6">
        <clipPath id="clipPath16" clipPathUnits="userSpaceOnUse">
          <path id="path18" d="M 0,38 38,38 38,0 0,0 0,38 Z" />
        </clipPath>
      </defs>
      <g transform="matrix(1.25,0,0,-1.25,0,47.5)" id="g10">
        <g id="g12">
          <g clipPath="url(#clipPath16)" id="g14">
            <g transform="translate(36,19)" id="g20">
              <path
                className="head"
                id="path22"
                style={{
                  fill: "#ffcc4d",
                  fillOpacity: 1,
                  fillRule: "nonzero",
                  stroke: "none"
                }}
                d="m 0,0 c 0,-9.389 -7.611,-17 -17,-17 -9.388,0 -17,7.611 -17,17 0,9.388 7.612,17 17,17 C -7.611,17 0,9.388 0,0"
              />
            </g>
            <g transform="translate(15,20.5)" id="g24">
              <path
                className="eye eye-left"
                id="path26"
                style={{
                  fill: "#664500",
                  fillOpacity: 1,
                  fillRule: "nonzero",
                  stroke: "none"
                }}
                d="M 0,0 C 0,-1.934 -1.119,-3.5 -2.5,-3.5 -3.881,-3.5 -5,-1.934 -5,0 -5,1.933 -3.881,3.5 -2.5,3.5 -1.119,3.5 0,1.933 0,0"
              />
            </g>
            <g transform="translate(28,20.5)" id="g28">
              <path
                className="eye eye-right"
                id="path30"
                style={{
                  fill: "#664500",
                  fillOpacity: 1,
                  fillRule: "nonzero",
                  stroke: "none"
                }}
                d="M 0,0 C 0,-1.934 -1.119,-3.5 -2.5,-3.5 -3.881,-3.5 -5,-1.934 -5,0 -5,1.933 -3.881,3.5 -2.5,3.5 -1.119,3.5 0,1.933 0,0"
              />
            </g>
            <g transform="translate(6.999,24)" id="g32">
              <path
                className="eye-brow-left"
                id="path34"
                style={{
                  fill: "#664500",
                  fillOpacity: 1,
                  fillRule: "nonzero",
                  stroke: "none"
                }}
                d="m 0,0 c -0.208,0 -0.419,0.065 -0.599,0.2 -0.442,0.331 -0.531,0.958 -0.2,1.4 3.262,4.35 7.616,4.4 7.8,4.4 0.552,0 1,-0.448 1,-1 C 8.001,4.449 7.556,4.002 7.005,4 6.85,3.998 3.436,3.914 0.801,0.4 0.604,0.138 0.304,0 0,0"
              />
            </g>
            <g transform="translate(31.001,24)" id="g36">
              <path
                className="eye-brow-right"
                id="path38"
                style={{
                  fill: "#664500",
                  fillOpacity: 1,
                  fillRule: "nonzero",
                  stroke: "none"
                }}
                d="M 0,0 C -0.305,0 -0.605,0.138 -0.801,0.4 -3.441,3.921 -6.862,3.999 -7.007,4 -7.557,4.006 -8.001,4.456 -7.998,5.005 -7.995,5.556 -7.551,6 -7.001,6 -6.817,6 -2.464,5.95 0.799,1.6 1.131,1.158 1.041,0.531 0.599,0.2 0.419,0.065 0.209,0 0,0"
              />
            </g>
            <g transform="translate(24.4854,9.1211)" id="g40">
              <path
                className="mouth"
                id="path42"
                style={{
                  fill: "#664500",
                  fillOpacity: 1,
                  fillRule: "nonzero",
                  stroke: "none"
                }}
                d="m 0,0 c -0.012,0.044 -1.146,4.379 -5.485,4.379 -4.341,0 -5.475,-4.335 -5.485,-4.379 -0.053,-0.213 0.043,-0.431 0.231,-0.544 0.188,-0.112 0.433,-0.086 0.596,0.06 0.01,0.007 1.014,0.863 4.658,0.863 3.589,0 4.617,-0.83 4.656,-0.863 0.095,-0.09 0.219,-0.137 0.344,-0.137 0.084,0 0.169,0.021 0.246,0.064 C -0.043,-0.445 0.055,-0.218 0,0"
              />
            </g>
            <g transform="translate(17,5)" id="g44">
              <path
                className="tear"
                id="path46"
                style={{
                  fill: "#5dadec",
                  fillOpacity: 1,
                  fillRule: "nonzero",
                  stroke: "none"
                }}
                d="m 0,0 c 0,-2.762 -2.238,-5 -5,-5 -2.762,0 -5,2.238 -5,5 0,2.762 4,10 5,10 1,0 5,-7.238 5,-10"
              />
            </g>
          </g>
        </g>
      </g>
    </svg>
  );
}
